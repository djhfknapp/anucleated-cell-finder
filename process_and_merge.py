...
def main():
    ...
    print("Aligning gene lists and filling missing values with 0...")
    # Get all unique gene columns from both dataframes
    all_genes = list(set(orig_counts.columns) | set(anuc_counts.columns))

    # Reindex both dataframes to include all genes, filling missing with 0
    orig_counts = orig_counts.reindex(columns=all_genes, fill_value=0)
    anuc_counts = anuc_counts.reindex(columns=all_genes, fill_value=0)

    # Ensure the column order of anuc_counts matches orig_counts for consistent concatenation
    # This reorders anuc_counts columns to match orig_counts, which now contains all genes.
